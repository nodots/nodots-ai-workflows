...
3. **Isolation via git worktrees.** Each worker runs in its own worktree of the target package repo (`/Users/kenr/Code/nodots-worktrees/<package>/<branch>` per the workspace convention), one `[READY]` PR per cell into that repo's `development`. This is the multi-repo reality: cells target specific package repos, never the workspace as a whole.
   **`git stash` is banned in worker worktrees.** A worktree isolates files; it does not isolate the stash. Linked worktrees of one repo share a single stack at `<common-dir>/refs/stash` — it is common state, not per-worktree state like `HEAD` or the index — so `stash@{0}` is whatever *any* worktree pushed most recently. A positional `git stash pop` therefore takes another worker's changes into the wrong tree, and it fails silently in both directions: the owner's worktree goes clean, which reads as "my changes vanished" rather than as a collision, and the popper's next build or suite run measures foreign changes. The trigger is the before/after measurement — stash, measure a baseline, pop, measure again — so brief workers off it: capture `git diff` to a patch file outside the tree, move untracked files aside, `git checkout -- .`, measure, then `git apply` the patch and restore the files. A patch file cannot reach into another worktree. If a stash exists anyway (a worker's habit, a tool that pushes one), recover it by reference and never by position: `git stash list --format='%gd %gs'`, find the entry whose message names this cell's branch, `git stash pop <that ref>`.
